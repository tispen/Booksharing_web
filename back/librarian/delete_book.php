...
<html>
    ...
    <?php
			...
			if($rows == 0)
				echo "<h2 align='center'>Нет доступных книг.</h2>";
			else
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					for($j=0; $j<6; $j++)
                            echo "<td>".$row[$j]."</td>";
                            echo "<td><div class='text-center'><a href='dltbook.php?id=".$row['isbn']."' style='color:#F66; text-decoration:none;'> Удалить</a></div></td>";
					echo "</tr>";
				}
			echo "</table>";
			echo "</form>";
